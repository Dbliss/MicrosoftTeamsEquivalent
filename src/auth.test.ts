...
test('Test successful returns authUserId', () => {
  clearV1();
  const result = authRegisterV1('z5364121@unsw.edu.au', 'banana', 'Thevindu', 'Hewa');
...
test('error when email is being used by another user', () => {
  ...
  const result = authRegisterV1('z5364121@unsw.edu.au', 'banana', 'Thev', 'Hewa');
  expect(result).toMatchObject({ error: 'error' });
});

test('password length less than 6 characters', () => {
  clearV1();
  const result = authRegisterV1('z5364121@unsw.edu.au', 'apple', 'Thev', 'Hewa');
  expect(result).toMatchObject({ error: 'error' });
});
...
test('Login succesful', () => {
  clearV1();
  authRegisterV1('z5364121@unsw.edu.au', 'banana', 'Thevindu', 'Hewa');
  const result = authLoginV1('z5364121@unsw.edu.au', 'banana');
  expect(result).toMatchObject({ authUserId: expect.any(Number) });
});

test('email does not belong to any user', () => {
  clearV1();
  const result = authLoginV1('z5364121@unsw.edu.au', 'apple');
  expect(result).toMatchObject({ error: 'error' });
});
